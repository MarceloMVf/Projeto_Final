// É obrigatório o uso de orientação a objetos e herança, - Criar um  objeto chamado monstros - Especie, 
//funções, 
//laços (while/for)
// e condicionais (if, elif, else), ou seja todo o conteúdo visto durante o módulo. - decisões
...
console.log (`\nMas nem tudo está perdido, você acaba de receber um bom contrato; Coroas de Novigrad por monstros para um mago colecionador.\n`)
console.log (`\nVocê deve fazer as escolhas de sua caçada, testando seus conhecimentos do mundo bruxo,  lembre se que sua remuneração depende disso. \n \n Sua missão inicia as 10 horas da manhã, você deve retornar antes do anoitecer para não encomodar os aldeões.\n`)

let personagem = prompt (`\nQual seu nome, caro bruxo?: `)
let name = personagem
...
console.log(`\n Já são ${i} horas, ${name} apenas se alimenta e se prepara para sair a caçada, levando consigo seu equipeamento, uma grande carroagem de carga e é claro, seu companheiro equino, pé de pano\n`)

console.log (`\nVocê sente a energia mística da seretonina, levanta-se e sobe no seu velho amigo, pé de pano.\n`)

console.log (`\n${name}: "Vamos pé de pano, estou ancioso por achar um Molinassauro e ganhar uma boa grana para comemorar a sexta da maldade, aquele evento da vila vizinha" \n`)


console.log (`\nAo chegar na floresta, não foi díficil encontrar pistas, com seu sentido bruxo, ouve ganidos e latidos em uma direção. No sentido oposto, consegue ver patas grandes, porém é díficil distinguir de que criatura, já que estão ofoscudas pela chuva da noite passada.\n`)
...
console.log (`Você adquiriu ${carteira} coroas de Novigrad!`);
...
if (carteira > 0){
    ...
    console.log (`Não é hoje que você vai curtir a sexta da maldade, como punição por não cumprir o contrato, você ficará codando o final de semana inteiro!`)
}